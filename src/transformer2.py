# modified from: https://towardsdatascience.com/the-time-series-transformer-2a521a0efad3
...
class AttentionBlock(keras.Model):
    def __init__(self, name='AttentionBlock', num_heads=2, head_size=128, ff_dim=None, dropout=0, **kwargs):
        ...
        self.attention = layers.MultiHeadAttention(key_dim=head_size, num_heads=num_heads, dropout=dropout)
        self.attention_dropout = layers.Dropout(dropout)
        self.attention_norm = layers.LayerNormalization(epsilon=1e-6)

        self.ff_conv1 = layers.Conv1D(filters=ff_dim, kernel_size=1, activation='relu')
        # self.ff_conv2 at build()
        self.ff_dropout = layers.Dropout(dropout)
        self.ff_norm = layers.LayerNormalization(epsilon=1e-6)

    def build(self, input_shape):
        self.ff_conv2 = layers.Conv1D(filters=input_shape[-1], kernel_size=1) 
